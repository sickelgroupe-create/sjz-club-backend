...
完整的新服务器初始化入口为 [deploy/README.md](deploy/README.md)。所有 SQL 集中于 [database/](database/README.md)：`001-schema.sql` 和 `002-seed.sql` 已在隔离空库中重新导入验证，包含 74 张表及菜单、角色权限、字典和必要配置；管理员需设置自己的密码后启用。环境变量、Nginx、服务模板和导入工具在 deploy/。
