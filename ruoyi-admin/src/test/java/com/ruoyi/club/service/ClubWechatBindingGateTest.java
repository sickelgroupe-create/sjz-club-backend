...
class ClubWechatBindingGateTest {
 ...
 @Test void restrictedRefreshCannotUpgradeBeforeBinding(){when(db.queryForList(contains("select t.id,t.user_id,t.token_type,u.status"),anyString())).thenReturn(Arrays.asList(row("id",2L,"user_id",68L,"token_type","bind_refresh","status","active")));doReturn(row("id",68L)).when(auth).userView(68L);Map<String,Object> r=auth.refresh(row("refreshToken","limited-refresh"));assertEquals(false,r.get("wechatBound"));verify(db).update(contains("insert into club_user_token"),eq(68L),eq("binding"),anyString(),any(java.sql.Timestamp.class));verify(db,never()).update(contains("insert into club_user_token"),eq(68L),eq("access"),anyString(),any(java.sql.Timestamp.class));}
