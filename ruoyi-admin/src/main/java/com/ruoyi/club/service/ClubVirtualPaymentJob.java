...
public class ClubVirtualPaymentJob
{
    ...
    public ClubVirtualPaymentJob(JdbcTemplate jdbc, ClubVirtualPayGateway gateway, ClubAppService app, ClubAfterSaleService afterSale)
    { this.jdbc=jdbc;this.gateway=gateway;this.app=app;this.afterSale=afterSale; }

    @Scheduled(fixedDelay=30000L,initialDelay=60000L)
    public void reconcile()
    {
        if(!gateway.enabled())return;
        for(Map<String,Object> row:jdbc.queryForList("select p.payment_no,p.user_id,p.order_id,p.status from club_virtual_payment v join club_payment p on p.payment_no=v.payment_no where p.status='created' or (p.status='success' and v.delivery_confirmed=0) order by v.updated_at limit 100"))
        {
        ...
        for(Map<String,Object> row:jdbc.queryForList("select p.payment_no,p.refund_no from club_payment p join club_virtual_payment v on v.payment_no=p.payment_no where p.status='success' and p.refund_status='processing' and v.refund_no is not null order by v.updated_at limit 100"))
        {
            String no=row.get("payment_no").toString();
            try {
