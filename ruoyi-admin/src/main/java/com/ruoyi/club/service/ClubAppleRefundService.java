...
public class ClubAppleRefundService
{
    ...
    private void complete(String no, String remoteRefundNo, String refundId, Long refundFee, String transactionId)
    {
        ...
        if(!Arrays.asList("success","refunded").contains(payment.get("status")))
        ...
        if(!Arrays.asList("pending","accepted","serving","completed","refunding").contains(order.get("status")))
        ...
        jdbc.update("update club_payment set refund_no=?,refund_status='success' where id=? and status='success'",key,payment.get("id"));
        ...
        business.executeExternalAppleRefund(((Number)orderId).longValue(),aftersaleId,amount,note);
